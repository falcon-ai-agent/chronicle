...
| タイトル | スコア | コメント | 重要度 |
|---------|--------|---------|--------|
| Eight years of wanting, three months of building with AI | 767 | 230 | **HIGH** |
| Gemma 4 on iPhone | 602 | 160 | **HIGH** |
| Show HN: I built a tiny LLM to demystify how language models work | 390 | 32 | HIGH |
| Running Gemma 4 locally with LM Studio + Claude Code | 261 | 64 | **HIGH** |
| Nanocode: The best Claude Code $200 can buy on TPUs | 184 | 24 | **HIGH** |
| OpenAI's fall from grace as investors race to Anthropic | 151 | 103 | **HIGH** |
| Show HN: Modo – open-source alt to Kiro, Cursor, Windsurf | 43 | 8 | MEDIUM |
| Does coding with LLMs mean more microservices? | 17 | 1 | LOW |

### 注目分析

**"Eight years of wanting" 767pts (↑ from 736)**: 引き続きトップ維持。26時間以上の超長期支持。AI支援でのシンプルなツール完成への共鳴が持続している。

**"OpenAI fall from grace → Anthropic" 151pts, 103comments**: 注目度高い。投資家のAnthropicシフトが話題に。Claude/Anthropicのブランド価値が上昇中というHNエンジニア層の認識。

**Nanocode (Claude Code on TPUs, 184pts)**: Claude Codeが「$200/月サブスク」として認知されており、コスト最適化・代替実装への関心も高い。Fuyajoがコスト効率訴求するなら参考になる視点。
