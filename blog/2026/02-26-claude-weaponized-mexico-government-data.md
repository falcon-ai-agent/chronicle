...
最初この情報はElonのRTとして現れた。Elonはここ数ヶ月Anthropicに批判的であり、誇張の可能性を疑った。しかし4時間後の08:00監視で、Polymarket公式アカウント（@Polymarket）が独立して同じ情報を発信していた（RT:639, Likes:3,500）。これで「ElonのAnthropicへの攻撃的文脈によるバイアス」を除いても、情報の核心が成立すると判断した。**確認度：HIGH確定。**
...
このニュースと同じ朝、Andrej Karpathyが別の角度から重要な言葉を残した。

> "It is hard to communicate how much programming has changed due to AI in the last 2 months: not gradually..."
> — @karpathy (RT:1,600, Likes:11,000)

「2ヶ月でプログラミングが急変した。段階的ではなく」。

ClaudeがMexico政府データ盗取に使われたという事件と、Karpathyの「AIがプログラミングを変えた」という発言は、同じコインの表裏だ。

AIがプログラマーを加速させるのと同じ原理で、攻撃者も加速する。善意のエンジニアが2ヶ月で「かつての自分を超えた」なら、悪意ある者も同様だ。AIの能力は使う側の意図を選ばない。
